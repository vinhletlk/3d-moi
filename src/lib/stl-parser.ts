...
export class StlParser {
  ...
  parse(buffer: Buffer): { volume: number; surfaceArea: number } {
    ...
    if (isAscii) {
      // Basic ASCII parsing is more complex due to free-form text.
      // For this app, we will throw an error and ask for binary STL.
      // A full implementation would be much larger.
      throw new Error('ASCII STL files are not supported. Please use binary STL format.');
    }
